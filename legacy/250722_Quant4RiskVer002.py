"""
ETF强化型量化交易策略 Ver 0.02
250722_Quant4RiskVer002.py
基于 市场趋势判断，结合均线、MACD、KDJ、布林带、RSI和成交量分析的高风险高回报量化交易策略，通过server酱推送通知到绑定微信上。
作者：Michael Yuan
"""
...
class TradingParameters:
    ...
    # 风险管理参数
    risk_pct_per_trade: float = 0.30  # 从0.25提高到0.30
    max_daily_loss: float = 0.40      # 从0.35提高到0.40
    max_drawdown: float = 0.70        # 从0.60提高到0.70
    ...
    # 获利了结参数
    profit_trigger_1: float = 0.40   # 从0.25提高到0.40
    profit_trigger_2: float = 0.80   # 从0.50提高到0.80

    td_require_regime: bool = True

    # 止损参数
    stop_atr: float = 4.0            # 从3.0提高到4.0
    force_min_stop_pct: float = 0.15  # 从0.12提高到0.15
    trail_stop_mult: float = 3.0
    profit_lock_pct: float = 0.5
    exit_confirm_threshold: int = 1

    use_intraday: bool = True         # 使用日内交易
    default_interval: str = "1h"     # 默认15分钟K线
    hot_sector_bonus: float = 2.0     # 热门板块加成
    momentum_threshold: float = 0.05  # 动量阈值5%
    ...
    min_data_points: int = 300  # 最小数据点数（降低要求）
...
class MarketState:
    ...
    def get_buy_signal(self) -> bool:
        ...
        try:
            ...
            # ===== 核心改进：降低进场门槛 =====
            # 快线必须在慢线上方的条件放宽
            if len(self.strategy.ma_fast) < 2 or len(self.strategy.ma_slow) < 2:
                return False

            # 修改1：只要快线向上就考虑
            ma_fast_rising = self.strategy.ma_fast[0] > self.strategy.ma_fast[-1]

            # 修改2：价格在快线附近即可（不必在上方）
            price_near_fast = abs(current_price - self.strategy.ma_fast[0]) / self.strategy.ma_fast[0] < 0.03

            # ===== 突破信号（放宽条件）=====
            # 10日新高突破（从20日改为10日）
            if len(self.strategy.data.high) >= 10:
                recent_high_10 = max(self.strategy.data.high.get(size=10)[:-1])
                if current_price > recent_high_10:
                    # 成交量确认放宽到1.2倍
                    if hasattr(self.strategy, 'volume_ma') and self.strategy.volume_ma[0] > 0:
                        ...
                        if volume_ratio > 1.2:  # 从1.5降到1.2
                            self.strategy.log(f"10日新高突破信号 - 价格{current_price:.2f} > {recent_high_10:.2f}", level="CRITICAL")
                            return True

            # ===== 动量信号（新增）=====
            if len(self.strategy.data.close) >= 5:
                momentum_5d = (current_price / self.strategy.data.close[-5] - 1)
                if momentum_5d > 0.05 and ma_fast_rising:  # 5天涨5%且均线向上
                    self.strategy.log(f"动量爆发信号 - 5日涨幅{momentum_5d*100:.1f}%", level="CRITICAL")
                    return True

            # ===== 强势股特征（放宽）=====
            if len(self.strategy.data.close) >= 20:
                # 20天涨幅超过10%（从50天20%改为20天10%）
                price_20d_ago = self.strategy.data.close[-20]
                stock_return = (current_price / price_20d_ago - 1)

                if stock_return > 0.10:
                    # 检查是否从短期低点反弹
                    recent_low_3 = min(self.strategy.data.low.get(size=3))  # 从5天改为3天
                    bounce = (current_price / recent_low_3 - 1)

                    if bounce > 0.01:  # 反弹1%即可（从2%降低）
                        # RSI不能过低
                        if 30 < self.strategy.rsi[0] < 80:  # 放宽RSI范围
                            self.strategy.log(f"强势股反弹信号 - 20日涨{stock_return*100:.1f}%, 反弹{bounce*100:.1f}%", level="CRITICAL")
                            return True

            # ===== MACD金叉（放宽）=====
            if len(self.strategy.macd.macd) > 2 and len(self.strategy.macd.signal) > 2:
                # MACD上穿信号线（不要求在0轴上方）
                if (self.strategy.macd.macd[-1] <= self.strategy.macd.signal[-1] and
                    self.strategy.macd.macd[0] > self.strategy.macd.signal[0]):
                    self.strategy.log("MACD金叉信号", level="CRITICAL")
                    return True

            # ===== 简单趋势信号（新增）=====
            if ma_fast_rising and price_near_fast and self.strategy.rsi[0] > 50:
                self.strategy.log("简单趋势信号触发", level="CRITICAL")
                return True
...
class StopManager:
    ...
    def __init__(self, params):
        ...
        # 新增：止损缓冲机制
        self.stop_triggered_bar = None  # 首次触发止损的K线
        self.stop_trigger_count = 0     # 连续触发次数
        self.stop_price_level = None    # 止损价位
        self.stop_reason = None         # 止损原因
    ...
    def _check_basic_stop(self, current_price, trend_strength):
        ...
        try:
            ...
            # 获取当前波动率
            current_volatility = self.strategy.volatility[0] if hasattr(self.strategy, 'volatility') else 0
            avg_volatility = np.mean([self.strategy.volatility[-i] for i in range(min(20, len(self.strategy.volatility)))])

            # 高波动期放宽止损
            volatility_mult = 1.0
            if current_volatility > avg_volatility * 1.5:
                volatility_mult = 1.5  # 从1.3提高到1.5，更宽松
                self.strategy.log(f"高波动期 - 止损放宽50%", level="DEBUG")
            # 添加：2025年热门股票额外放宽
            symbol = self.strategy.symbol.upper()
            hot_stocks = ParamAccessor.get_param(self, 'hot_stocks_2025')
            if symbol in hot_stocks:
                volatility_mult *= 1.2  # 热门股票再放宽20%
                self.strategy.log(f"热门股票{symbol} - 止损额外放宽20%", level="DEBUG")

            # ===== 日内模式特殊处理 =====
            if self.strategy.trading_params.intraday_mode:
                # 日内交易使用更宽松的止损
                atr = self.strategy.atr[0]

                # 使用更宽的ATR倍数
                if holding_days <= 1:  # 当日
                    atr_mult = 3.0  # 从2.0提高到3.0
                else:
                    atr_mult = 4.0  # 从2.5提高到4.0

                atr_stop = entry_price - (atr * atr_mult * volatility_mult)

                # 固定百分比止损（更宽松）
                if holding_days <= 1:
                    pct_stop = entry_price * 0.90  # 10%止损（从5%放宽）
                elif holding_days <= 5:
                    pct_stop = entry_price * 0.88  # 12%止损（从8%放宽）
                else:
                    pct_stop = entry_price * 0.85  # 15%止损（从12%放宽）

                # 使用更严格的止损
                stop_price = max(atr_stop, pct_stop)

                # 盘中实时判断，不需要缓冲机制
                if current_price < stop_price and current_pnl < -0.03:  # 亏损超过3%
                    self.strategy.log(
                        f"日内实时止损触发 - 当前价{current_price:.2f} < 止损价{stop_price:.2f}, "
                        f"亏损{current_pnl:.1%}",
                        level="CRITICAL"
                    )
                    return True
                ...
                # 基础止损：根据ATR和波动率动态调整
                atr = self.strategy.atr[0]
                atr_stop = entry_price - (atr * 3.0 * volatility_mult)

                # 时间止损：持有时间越长，止损越宽松
                if holding_days <= 5:
                    time_stop = entry_price * (0.88 / volatility_mult)  # 考虑波动率
                elif holding_days <= 20:
                    time_stop = entry_price * (0.82 / volatility_mult)
                else:
                    time_stop = entry_price * (0.75 / volatility_mult)

                # 使用更宽松的止损
                stop_price = min(atr_stop, time_stop)

                # ===== 盈利保护（提高门槛）=====
                if current_pnl > 0.30:  # 盈利30%以上才保护
                    high_since_entry = getattr(self.strategy, 'high_since_entry', entry_price)
                    trailing_stop = high_since_entry * 0.82
                    stop_price = max(stop_price, trailing_stop)

                    if current_price < stop_price:
                        # 使用缓冲机制
                        return self._check_stop_buffer(stop_price, current_price, "盈利保护止损")

                # ===== 趋势破坏止损（增加缓冲）=====
                trend_break_conditions = [
                    current_price < self.strategy.ma_slow[0] * 0.92,
                    self.strategy.ma_fast[0] < self.strategy.ma_slow[0],
                    current_pnl < -0.15,
                    self.strategy.macd.macd[0] < self.strategy.macd.signal[0] * 0.9  # 新增MACD确认
                ]

                # 需要至少3个条件满足才触发
                if sum(trend_break_conditions) >= 3:
                    return self._check_stop_buffer(stop_price, current_price, "趋势严重破坏止损")

                # 基础止损判断
                if current_price < stop_price:
                    return self._check_stop_buffer(stop_price, current_price, "基础止损")

                # 如果价格回升，重置缓冲计数
                if self.stop_triggered_bar is not None and current_price > self.stop_price_level * 1.02:
                    self._reset_stop_buffer()
                    self.strategy.log(f"价格回升，止损缓冲重置", level="INFO")
    ...
    def _check_stop_buffer(self, stop_price, current_price, reason):
        """止损缓冲机制 - 避免被短期波动洗出"""
        current_bar = len(self.strategy)

        # 首次触发
        if self.stop_triggered_bar is None:
            self.stop_triggered_bar = current_bar
            self.stop_trigger_count = 1
            self.stop_price_level = stop_price
            self.stop_reason = reason
            self.strategy.log(
                f"{reason}触发 (1/3) - 当前价{current_price:.2f} < 止损价{stop_price:.2f}",
                level="INFO"
            )
            return False  # 首次不执行

        # 检查是否连续触发
        if current_bar - self.stop_triggered_bar <= 3:  # 3天内
            if current_price < self.stop_price_level:
                self.stop_trigger_count += 1
                self.strategy.log(
                    f"{self.stop_reason}确认 ({self.stop_trigger_count}/3) - 价格持续低于止损线",
                    level="INFO"
                )

                # 需要连续3天确认才执行止损
                if self.stop_trigger_count >= 3:
                    self.strategy.log(f"{self.stop_reason}最终确认 - 连续3天低于止损线", level="CRITICAL")
                    return True
            else:
                # 价格回升，减少计数但不立即重置
                self.stop_trigger_count = max(0, self.stop_trigger_count - 1)
                if self.stop_trigger_count == 0:
                    self._reset_stop_buffer()
        else:
            # 超过3天窗口，重置
            self._reset_stop_buffer()
    ...
    def _reset_stop_buffer(self):
        """重置止损缓冲状态"""
        self.stop_triggered_bar = None
        self.stop_trigger_count = 0
        self.stop_price_level = None
    ...
    def _check_trailing_stop(self, current_price, market_regime):
        ...
        try:
            ...
            # 触发条件
            if drop_from_peak > max_allowed_drop and profit_pct > trailing_stop_min_profit:
                # 使用缓冲机制
                return self._check_stop_buffer(
                    high_since_entry * (1 - max_allowed_drop),
                    current_price,
                    f"追踪止损（从高点回撤{drop_from_peak:.1%}）"
                )
...
class PositionManager:
    ...
    def _check_enhanced_profit_levels(self, profit_pct):
        ...
        # 大幅提高获利门槛
        if profit_pct < 0.25:  # 从0.15提高到0.25（低于25%不考虑止盈）
            return 0
        ...
        if trend_intact:
            # 趋势完好：只做小幅获利
            if profit_pct > 1.00:  # 从0.50提高到1.00（盈利超过100%）
                return int(current_position_size * 0.20)  # 从0.25降到0.20
            elif profit_pct > 0.60:  # 从0.30提高到0.60（盈利超过60%）
                return int(current_position_size * 0.10)  # 从0.15降到0.10
            else:
                return 0  # 继续持有
        else:
            # 趋势破坏：根据盈利水平决定
            if profit_pct > 0.50:  # 从0.30提高到0.50
                return int(current_position_size * 0.40)  # 从0.50降到0.40
            elif profit_pct > 0.35:  # 从0.20提高到0.35
                return int(current_position_size * 0.25)  # 从0.33降到0.25
            else:
                return int(current_position_size * 0.15)  # 从0.25降到0.15
        ...
        try:
            ...
            if self.strategy.position:
                ...
                if trend_score >= 3 and current_position_ratio < 0.6:
                    # 修复：直接使用self而不是self.position_manager
                    batches = getattr(self, 'added_batches', 0)  # 关键修复
                    add_ratio = 0.2 * (0.7 ** batches)  # 保持原逻辑
                    target_fraction = min(add_ratio, 0.8 - current_position_ratio)  # 保持原逻辑
                else:
                    target_fraction = 0  # 确保变量被定义
            else:
                # 新建仓位：根据趋势强度
                if trend_score >= 4:
            ...
            # 确保有意义的交易规模
            min_shares = max(50, int(portfolio_value * 0.05 / current_price))
            if position_size < min_shares:
                return 0

            self.strategy.log(
                f"仓位计算 - 趋势得分:{trend_score}, 目标仓位:{target_fraction:.1%}, 股数:{position_size}",
                level="CRITICAL"
            )
...
class EnhancedStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        # 添加数据长度检查
        if len(self) < self.trading_params.long_ma:
            self.log(f"数据不足，当前长度: {len(self)}，需: {self.trading_params.long_ma}", level="DEBUG")
            return
        ...
        try:
            ...
            if self.position and not is_small_position:
                ...
                # 16.5 获取TD参数（修正：使用ParamAccessor）
                td_count = ParamAccessor.get_param(self, 'td_count')
                ...
                # 16.10 TD序列必要性检查（激进版放宽要求）
                if td_count > 0:
                    td_satisfied = hasattr(self.market_state, 'td_up') and self.market_state.td_up >= td_count
                    # 激进版：即使TD未满足，如果有强烈信号也可以买入
                    if not td_satisfied and not base_buy:
                        self.log(
                            f"TD序列未满足({getattr(self.market_state, 'td_up', 0)} < {td_count})，等待更强信号",
                            level="INFO"
                        )
                        return
...
def preprocess_intraday_data(df: pd.DataFrame, trading_params: TradingParameters) -> pd.DataFrame:
    ...
    # 强制过滤盘前盘后（Alpha Vantage数据特殊处理）
    if 'alpha' in str(df.get('source', '')).lower() or original_len > 1000:
        # Alpha Vantage数据通常包含完整的4:00-20:00数据
        # 强制只保留正常交易时段 9:30-16:00
