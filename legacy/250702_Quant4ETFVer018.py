"""
ETF强化型量化交易策略 Ver 0.18
250702_Quant4ETFVer018.py
基于 TD序列 和 市场趋势判断，结合均线、MACD、RSI和成交量分析的综合性 ETF量化交易策略
作者：Michael Yuan
"""
...
class MarketState:
    ...
    # 位置：在 MarketState 类中，替换 get_buy_signal 方法
    # 上下文：位于 update_td_sequence 之后
    def get_buy_signal(self) -> bool:
        ...
        try:
            ...
            if not self.strategy.position:
                ...
                if bars_since_last_trade > 60 and total_signal > effective_threshold * 0.7 and trend_strength > 0:
                    result = True
    ...
    def get_market_regime(self, force_update=False):
        ...
        try:
            ...
            trend_strength = self.get_trend_strength(level_recalculate=True)
            strong_uptrend_threshold = ParamAccessor.get_param(self, 'strong_uptrend_threshold')
            uptrend_threshold = ParamAccessor.get_param(self, 'uptrend_threshold')
            sideways_threshold = ParamAccessor.get_param(self, 'sideways_threshold')
            downtrend_threshold = ParamAccessor.get_param(self, 'downtrend_threshold')
