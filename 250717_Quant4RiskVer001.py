"""
ETF强化型量化交易策略 Ver 0.01
250717_Quant4RiskVer001.py
...
class TradingParameters:
    ...
    # 技术指标参数
    fast_ma: int = 5           # 保持5
    slow_ma: int = 10          # 从8改为10
    mid_ma: int = 20           # 从15改为20
    long_ma: int = 50          # 从30改为50
    macd_fast: int = 8         # 从12降到8
    macd_slow: int = 17        # 从26降到17
    macd_signal: int = 6       # 从9降到6
    atr_period: int = 7        # 从14降到7
    rsi_period: int = 9        # 从14降到9
    volatility_window: int = 10 # 从20降到10

    # 新增动量参数
    momentum_period: int = 5    # 从10降到5
    volume_surge_mult: float = 2.5  # 新增：成交量爆发倍数
    breakout_period: int = 20   # 新增：突破周期
    ...
    # 风险管理参数
    risk_pct_per_trade: float = 0.25  # 从0.15提高到0.25
    max_daily_loss: float = 0.35      # 从0.25提高到0.35
    max_drawdown: float = 0.60        # 从0.50提高到0.60
    trade_loss_pct: float = 0.15
    equity_risk: float = 0.3
    min_bars_between_trades: int = 1

    # 仓位参数
    base_position_pct: float = 0.6    # 从0.4提高到0.6
    max_position_pct: float = 0.95    # 从0.8提高到0.95
    min_position_size: int = 100      # 从10提到100

    # 获利了结参数
    profit_trigger_1: float = 0.25   # 从0.15提高到0.25
    profit_trigger_2: float = 0.50   # 从0.30提高到0.50

    td_require_regime: bool = True


    # 止损参数
    stop_atr: float = 3.0            # 从2.0提高到3.0
    force_min_stop_pct: float = 0.12  # 从0.05提高到0.12
    trail_stop_mult: float = 3.0
    profit_lock_pct: float = 0.5
    exit_confirm_threshold: int = 1

    use_intraday: bool = True         # 使用日内交易
    default_interval: str = "15m"     # 默认15分钟K线
    ...
    def _check_basic_stop(self, current_price, trend_strength):
        ...
        try:
            ...
            if current_volatility > avg_volatility * 1.5:
                volatility_mult = 1.3  # 波动率高时放宽30%
                self.strategy.log(f"高波动期 - 止损放宽30%", level="DEBUG")
