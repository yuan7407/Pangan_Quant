"""
ETF强化型量化交易策略 Ver 0.17
250604_Quant4ETFVer017.py
作者：Michael Yuan
"""
...
class TradingParameters:
    ...
    # 仓位参数
    base_position_pct: float = 0.3
    min_position_size: int = 10
    max_position_pct: float = 0.6

    # 获利了结参数
    profit_trigger_1: float = 0.15
    profit_trigger_2: float = 0.25
    ...
    max_single_loss: float = 0.02  # 单笔交易最大可承受损失（2%）
...
class TradeManager:
    ...
    def add_trade(self, trade: Dict) -> None:
        try:
            ...
            if not isinstance(trade, dict):
                if self.strategy:
                    self.strategy.log("添加交易记录失败: 传入的交易不是字典类型", level="ERROR")
                return
            ...
            if entry_price <= 0 or entry_price > 10000:
                self.strategy.log(f"跳过异常交易记录: 价格异常 {entry_price}", level="WARNING")
                return

            if abs(size) <= 0 or abs(size) > 50000:
                self.strategy.log(f"跳过异常交易记录: 数量异常 {size}", level="WARNING")
                return
            ...
            if trade.get('size', 0) <= 0 and trade.get('orig_size', 0) <= 0:
                if self.strategy:
                    self.strategy.log(f"跳过无效交易记录: 数量为0", level="DEBUG")
                return
    ...
    def get_statistics(self) -> Dict:
        """获取完整统计数据"""
        # 检查缓存
        current_count = len(self.executed_trades)
        if (self._last_update == current_count and
            self._stats_cache):
        ...
        stats = {
            ...
            'profit_factor': (
                abs(sum(t['pnl'] for t in self.executed_trades if t['pnl'] > 0)) /
                abs(sum(t['pnl'] for t in self.executed_trades if t['pnl'] < 0))
                if any(t['pnl'] < 0 for t in self.executed_trades) else float('inf')
            ),

            # 交易表现
            'avg_profit': self.avg_profit,
            'max_profit': self.max_profit_trade,
            'max_loss': self.max_loss_trade,
            'max_winning_streak': self.max_winning_streak,
            'max_losing_streak': abs(self.max_losing_streak),
            ...
            'min_holding_period': (
                ...
                if self.min_holding_period != float('inf') else 0
            ),
            ...
            'avg_daily_pnl': (
                sum(self.daily_pnl.values()) / len(self.daily_pnl)
                if self.daily_pnl else 0
            )
        }

        # 更新缓存
        self._stats_cache = stats
        self._last_update = current_count
...
class MarketState:
    ...
    def _calculate_trend_strength(self):
        ...
        try:
            ...
            except (AttributeError, IndexError) as data_err:
                self.strategy.log(f"获取价格数据错误: {str(data_err)}")
                return 1.0
            ...
            if not long_term_trend_up:
                ...
                price_score = min(price_score, sideways_threshold)

            # 3. 短期收益判断
            trend_price_up_score = ParamAccessor.get_param(self, 'trend_price_up_score')
            if returns.get(1, 0) > -0.01:  # ETF允许1%的小幅负收益
                price_score += trend_price_up_score * 0.8
            elif returns.get(3, 0) > 0:  # 或3天收益为正
                price_score += trend_price_up_score * 0.6
            ...
            if len(returns) >= 3:
                ...
                if returns.get(1, 0) > returns.get(3, 0):
                    price_score += trend_price_accel_score
                # 短期收益率显著弱于中期收益率（减速）
                elif returns.get(1, 0) < returns.get(3, 0) - 0.02:
                    price_score -= trend_price_decel_penalty
                ...
                if returns.get(10, 0) > 0.05:  # 10天收益超过5%
                    price_score += trend_long_up_score
                elif returns.get(20, 0) < -0.05:  # 20天跌幅超过5%
                    price_score -= trend_long_down_penalty
            ...
            if current_price > self.strategy.ma_fast[0]:
                price_score += trend_price_above_fast_score
            elif current_price < self.strategy.ma_fast[0] * 0.95:
                price_score -= trend_price_below_fast_penalty

            if current_price > self.strategy.ma_slow[0]:
                price_score += trend_price_above_slow_score
            elif current_price < self.strategy.ma_slow[0] * 0.95:
                price_score -= trend_price_below_slow_penalty
            ...
            try:
                ...
                if (self.strategy.ma_fast[0] > self.strategy.ma_mid[0] and
                    ...
                    price_score += trend_ma_bull_score
                elif (self.strategy.ma_fast[0] < self.strategy.ma_mid[0] and
                      self.strategy.ma_mid[0] < self.strategy.ma_slow[0]):
                    price_score -= trend_ma_bear_penalty
                ...
                if len(self.strategy.ma_fast) > 2 and len(self.strategy.ma_slow) > 2:
                    ...
                    if (self.strategy.ma_fast[-2] <= self.strategy.ma_slow[-2] and
                        ...
                        price_score += trend_ma_golden_cross_score
                    # 死叉检测
                    elif (self.strategy.ma_fast[-2] >= self.strategy.ma_slow[-2] and
                          self.strategy.ma_fast[-1] < self.strategy.ma_slow[-1]):
                        price_score -= trend_ma_death_cross_penalty
            except:
                pass

            # 记录计算过程
            self.strategy.log(
                f"趋势计算 - 价格评分: {price_score:.1f}, 动量评分: {momentum_score:.1f}",
                level="INFO"
            )
            ...
            self.strategy.log(f"Trend strength calculation error: {str(e)}")
            return 1.0
    ...
    def update_td_sequence(self):
        ...
        try:
            ...
            if current_price > prev_price:
                ...
                # 记录参数使用
                self.strategy.log(f"TD序列更新 - td_up: {self.td_up}, td_count: {td_count}, td_check_days: {td_check_days}", level="DEBUG")
                ...
                # 记录详细的检查过程
                self.strategy.log(f"TD序列均线检查: 使用td_check_days={td_check_days}, 条件=[{', '.join([str(c) for c in ma_check_list])}], 结果={ma_check}", level="DEBUG")
                ...
                # 明确记录td_count比较过程
                self.strategy.log(f"TD序列买入条件检查: td_up({self.td_up}) >= td_count({td_count})? {self.td_up >= td_count}", level="DEBUG")
                ...
                if self.td_up >= td_count and dip_from_high >= dip_base_pct:
                    ...
                    # 记录详细的判断条件
                    self.strategy.log(f"TD序列市场状态影响: 状态={market_regime}, 趋势要求={trend_required}, 实际趋势={trend_strength}, 结果={trend_ok}", level="DEBUG")
            ...
            # 记录整体状态
            self.strategy.log(f"TD序列更新完成 - td_up={self.td_up}, td_down={self.td_down}, 信号强度={getattr(self, 'td_signal_strength', 0)}", level="DEBUG")
    ...
    def get_buy_signal(self) -> bool:
        ...
        try:
            ...
            if len(self.strategy.data.close) >= 10:
                ...
                # 提高下跌阈值，避免在小幅回调时买入
                if drop_5d < -0.08 or drop_10d < -0.12:  # 从-0.05/-0.08提高到-0.08/-0.12
                    # 检查成交量确认
                    ...
                    if hasattr(self.strategy, 'volume_ma') and self.strategy.volume_ma[0] > 0:
                        ...
                        volume_confirm = 1.2 < volume_ratio < 2.5  # 需要明显放量
            ...
            # 1. TD序列信号
            td_signal = 0.0
            if hasattr(self, 'td_signal_strength'):
                td_signal = self.td_signal_strength * td_sequence_weight
            elif getattr(self, 'buy_signal', False):
                td_signal = 1.0 * td_sequence_weight
            ...
            try:
                ...
                # 均线趋势
                ma_fast_up = self.strategy.ma_fast[0] > self.strategy.ma_fast[-5] if len(self.strategy.ma_fast) > 5 else False

                if price_above_fast and price_above_slow and ma_fast_up:
                    trend_signal = 1.0
                elif price_above_fast and ma_fast_up:
                    trend_signal = 0.8
                elif price_above_fast or price_above_slow:
                    trend_signal = 0.4
            ...
            try:
                current_price = self.strategy.data.close[0]
                current_open = self.strategy.data.open[0]
                prev_close = self.strategy.data.close[-1]

                bullish_candle = current_price > current_open
                price_above_prev = current_price > prev_close

                if bullish_candle and price_above_prev:
                    pattern_signal = 1.0
                elif bullish_candle or price_above_prev:
                    pattern_signal = 0.5
            ...
            try:
                ...
                volume_ma = self.strategy.volume_ma[0]

                if volume_ma > 0:
                    volume_ratio = current_volume / volume_ma
                    if volume_ratio > (1.0 + base_volume_mult):
                        volume_signal = 1.0
            ...
            if market_regime == "strong_uptrend" and trend_strength > 8:
                market_bonus = 0.6
            elif market_regime == "uptrend" and trend_strength > 4:
                market_bonus = 0.5
            ...
            # 1. 趋势追踪买入（保留原有逻辑）
            if not result and market_regime in ["strong_uptrend", "uptrend"]:
                if trend_strength > 10.0 and trend_signal > 0.5:
                    if hasattr(self.strategy, 'volume_ma') and self.strategy.volume_ma[0] > 0:
                        volume_ratio = self.strategy.data.volume[0] / self.strategy.volume_ma[0]
                        if volume_ratio > 0.8:
                            result = True
                            self.strategy.log(
                                f"趋势追踪买入 - 市场:{market_regime}, 趋势强度:{trend_strength:.1f}",
                                level="INFO"
                            )
            ...
            # 3. 空仓时间过长的处理
            if not self.strategy.position and not result:
                last_trade_bar = getattr(self.strategy, 'last_trade_bar', 0)
                bars_since_last_trade = len(self.strategy) - last_trade_bar

                if bars_since_last_trade > 60:
                    # 降低要求
                    if total_signal > effective_threshold * 0.7 and trend_strength > 0:
                        result = True
                        self.strategy.log(
                            f"空仓{bars_since_last_trade}天，降低买入要求",
                            level="INFO"
                        )
    ...
    def get_market_regime(self, force_update=False):
        ...
        try:
            ...
            try:
                if len(self.strategy.ma_fast) > 5:
                    ma_fast_up = self.strategy.ma_fast[0] > self.strategy.ma_fast[-5]
                if len(self.strategy.ma_slow) > 20:
                    ma_slow_up = self.strategy.ma_slow[0] > self.strategy.ma_slow[-20]
            except:
                pass

            # 如果趋势强度和技术指标严重不符，进行调整
            ...
            symbol = ""
            try:
                if hasattr(self.strategy, 'params') and hasattr(self.strategy.params, 'symbol'):
                    symbol = self.strategy.params.symbol
            except:
                pass

            # ETF倾向于上涨（保留原逻辑）
            if symbol and any(etf in symbol.upper() for etf in ['QQQ', 'SPY', 'IWM', 'DIA']):
                if regime == "sideways" and trend_strength > 0 and price_above_slow:
                    regime = "uptrend"

            # ===== 状态切换确认机制（简化但保留）=====
            if not hasattr(self, '_regime_change_counter'):
                self._regime_change_counter = 0
            if not hasattr(self, '_prev_regime'):
            ...
            # 只对极端状态变化要求确认
            extreme_change = (
                (self._prev_regime in ["strong_uptrend", "uptrend"] and regime == "downtrend") or
                (self._prev_regime == "downtrend" and regime in ["strong_uptrend", "uptrend"])
            )
            ...
            if extreme_change:
                if self._prev_regime != regime:
                    ...
                    self.strategy.log(f"极端状态变化需要确认: {self._prev_regime} → {regime} (1/{regime_change_confirmation_days})", level="INFO")
                else:
                    self._regime_change_counter += 1
                    if self._regime_change_counter >= regime_change_confirmation_days:
                        ...
                        self._regime_change_counter = 0
                        self.strategy.log(f"极端状态变化确认完成: → {regime}", level="CRITICAL")
                    else:
                        regime = self._prev_regime
            else:
                # 非极端变化立即生效
            ...
            if self._market_regime != regime:
                self.strategy.log(
                    f"市场状态变化: {self._market_regime} → {regime} (趋势强度:{trend_strength:.1f})",
                    level="INFO"
                )
            ...
            import traceback
            self.strategy.log(traceback.format_exc(), level="ERROR")
            return "sideways"

    def is_market_crash(self):
        """判断是否处于市场崩盘状态"""
        try:
            ...
            # 检查多个时间维度的下跌
            price_5d = (self.strategy.data.close[0] / self.strategy.data.close[-5] - 1) if len(self.strategy.data.close) > 5 else 0
            price_10d = (self.strategy.data.close[0] / self.strategy.data.close[-10] - 1) if len(self.strategy.data.close) > 10 else 0
            price_20d = (self.strategy.data.close[0] / self.strategy.data.close[-20] - 1) if len(self.strategy.data.close) > 20 else 0
            ...
            # 判断崩盘条件
            if price_5d < -0.10 and price_10d < -0.15 and volume_spike:  # 放宽到10%和15%
                self.strategy.log(
            ...
            for i in range(min(5, len(self.strategy.data.close)-1)):
                if self.strategy.data.close[-i] < self.strategy.data.close[-i-1]:
                    consecutive_down_days += 1
                else:
                    break

            if consecutive_down_days >= 4 and price_5d < -0.06:
                self.strategy.log(
                    f"连续下跌检测 - 连跌{consecutive_down_days}天, 累计跌{price_5d*100:.1f}%",
                    level="CRITICAL"
                )
                return True
            ...
            self.strategy.log(f"市场崩盘检测错误: {str(e)}")
            return False
        ...
        try:
            ...
            if current_drawdown < dynamic_max_drawdown:
                ...
                if not hasattr(self, '_drawdown_triggered'):
                    ...
                    # 仍在限制中
                    if days_since_trigger % 10 == 0:  # 每10天提醒一次
                        self.strategy.log(
                            f"回撤保护持续中 - 当前回撤: {current_drawdown:.2%}, "
                            f"已持续: {days_since_trigger}天",
                            level="INFO"
                        )
                    return True
            else:
            ...
            if self.strategy.last_trade_pnl < 0:
                ...
                # 普通亏损的处理
                elif days_since_loss > 10:
                    # 超过10天，自动清除亏损记录
                    self.strategy.log(f"亏损记录过期，重置风险状态", level="INFO")
                    self.strategy.last_trade_pnl = 0
                    self.strategy.last_loss_day = 0
                    return False
        ...
        try:
            ...
            if self.last_date != current_date:
                ...
                if self.last_date is not None:
                    self.strategy.log(
                        f"Daily summary - High: {getattr(self, 'daily_high', current_price):.2f}, "
                        f"Low: {getattr(self, 'daily_low', current_price):.2f}"
                    )
            ...
            if hasattr(self, 'indicators') and self.indicators:
                try:
                    ...
                    if hasattr(self, 'last_price'):
                        vol_estimate = abs(current_price - self.last_price) / self.last_price
                        self.vol_ma20 = (getattr(self, 'vol_ma20', vol_estimate) * 19 + vol_estimate) / 20
                        self.vol_high = max(getattr(self, 'vol_high', vol_estimate), vol_estimate)
...
class StopManager:
    ...
    def _check_basic_stop(self, current_price, trend_strength):
        ...
        try:
            ...
            if days_in_position >= stop_long_days:
                ...
                effective_stop_atr = stop_atr * stop_long_days_mult
            ...
            # 记录最低止损水平的影响
            self.strategy.log(
                f"最低止损水平计算 - 当前价格: {current_price:.2f} × {stop_min_level_mult:.2f} = {min_stop_level:.2f}",
                level="DEBUG"
            )
            ...
            # 记录详细的最终止损决策，显示所有参数的影响
            self.strategy.log(
                f"最终止损决策 - 持仓天数: {days_in_position}, "
                f"stop_atr: {stop_atr}{f' → {effective_stop_atr:.2f}' if effective_stop_atr != stop_atr else ''}, "
                f"stop_long_days_mult: {stop_long_days_mult}, "
                f"ETF倍数: {etf_multiplier:.1f}, "
                f"止损价: {final_stop:.2f}",
                level="DEBUG"
            )
    ...
    def _check_trailing_stop(self, current_price, market_regime):
        ...
        try:
            ...
            # 记录详细的乘数影响
            self.strategy.log(f"追踪止损乘数详情 - 基础:{base_trail:.2f}, 市场乘数:{trail_multiplier:.2f}, "
                             f"结果:{base_trail * trail_multiplier:.2f}", level="INFO")
            ...
            if orig_multiplier != trail_multiplier:
                self.strategy.log(f"趋势强度调整乘数: {orig_multiplier:.2f} -> {trail_multiplier:.2f}", level="INFO")
            ...
            if orig_factor != trail_stop_mult:
                self.strategy.log(f"持仓时间调整追踪因子: {orig_factor:.4f} -> {trail_stop_mult:.4f}", level="INFO")
            ...
            # 记录追踪止损计算详情
            self.strategy.log(f"追踪止损计算 - 历史最高: {high_since_entry:.2f}, 追踪因子: {trail_stop_mult:.4f}, "
                          f"计算得到trail_stop: {trail_stop:.2f}, 实际跌幅: {drop_from_peak:.2%}", level="INFO")

             # 快速下跌检测
            rapid_drop = False
            try:
                if len(self.strategy.data.close) > 5:
                    five_day_high = max([self.strategy.data.high[-i] for i in range(6)])
                    recent_drop = (five_day_high - current_price) / five_day_high
                    rapid_drop = recent_drop > trailing_stop_rapid_drop
                    self.strategy.log(f"快速下跌检测: 5天内下跌{recent_drop:.2%} > 阈值{trailing_stop_rapid_drop:.2%}? {rapid_drop}", level='INFO')
            except:
                pass
            ...
            # 快速下跌情况
            if rapid_drop:
                min_profit_to_stop *= 0.8
                min_drop_threshold *= 0.8
            ...
            # 各种情况的止损触发
            if rapid_drop and price_below_stop and drop_from_peak > trailing_stop_rapid_drop * 1.5:
                self.strategy.log(f"ETF追踪止损触发 - 快速下跌且价格低于止损位")
                return True
            elif holding_days > trailing_stop_long_holding and price_below_stop and drop_from_peak > min_drop_threshold:
                ...
                return True
            elif drop_from_peak > min_drop_threshold * 1.67:  # 极端回撤情况
...
class PositionManager:
    ...
    def _init_position_limits(self):
        ...
        # 记录乘数设置
        self.strategy.log(
            f"乘数设置 - strong_uptrend: {strong_uptrend_mult}, uptrend: {uptrend_mult}, "
            f"sideways: {sideways_mult}, downtrend: {downtrend_mult}",
            level="INFO"
        )
    ...
    def _handle_small_position(self, current_position_size, min_meaningful_trade,
        ...
        """处理小仓位整合"""
        # 修改：大幅放宽小仓位清理条件
        if abs_position_size < min_meaningful_trade * 2.0 and abs_position_size > 0:  # 从0.5改为2.0，扩大小仓位判定范围
            # 修改1：将持有时间要求从1年改为180天
            if holding_days > 180:  # 持有超过180天（6个月）
                # 修改2：将盈利阈值从50%改为20%
                if profit_pct > 0.20:  # 盈利超过20%
                    self.strategy.log(
                        f"长期小仓位获利了结 - 持仓({abs_position_size}股)，"
                    ...
                    return {
                        ...
                        'reason': 'Long-term small position profit taking'
                    }
                # 修改3：新增条件 - 持有超过1年且有任何盈利即可清理
                elif holding_days > 365 and profit_pct > 0.05:  # 1年且盈利5%
                    self.strategy.log(
                        f"超长期小仓位清理 - 持仓({abs_position_size}股)，"
                        f"持有{holding_days}天，盈利{profit_pct*100:.1f}%",
                        level="INFO"
                    )
                    return {
                        'action': 'sell',
                        'size': abs_position_size,
                        'reason': 'Extended holding small position cleanup'
                    }
                # 修改4：新增条件 - 持有超过2年，即使小幅亏损也清理
                elif holding_days > 730 and profit_pct > -0.10:  # 2年且亏损不超过10%
                    self.strategy.log(
                        f"超长期仓位强制清理 - 持仓({abs_position_size}股)，"
                        f"持有{holding_days}天，盈亏{profit_pct*100:.1f}%",
                        level="INFO"
                    )
                    return {
                        'action': 'sell',
                        'size': abs_position_size,
                        'reason': 'Force cleanup after 2 years'
                    }
                elif profit_pct < -0.20:  # 亏损阈值从30%改为20%
                    self.strategy.log(
                        f"长期小仓位止损 - 持仓({abs_position_size}股)，亏损{profit_pct*100:.1f}%",
                        level="INFO"
                    )
                    return {
                        'action': 'sell',
                        'size': abs_position_size,
                        'reason': 'Long-term small position stop loss'
                    }
        return None
            ...
            if self.added_batches > 0:
                ...
                if hasattr(self, '_buy_price_history') and self._buy_price_history:
                    ...
                    if current_price >= last_buy_price:
                        self.strategy.log(
                            f"加仓价格高于上次买入 - 当前{current_price:.2f} >= 上次{last_buy_price:.2f}",
                            level="DEBUG"
                        )
                        return False
                    ...
                    if actual_drop < min_price_drop:
                        self.strategy.log(
                            f"加仓跌幅不足 - 当前{current_price:.2f} vs 上次{last_buy_price:.2f}, "
                            f"跌幅{actual_drop*100:.2f}% < 要求{min_price_drop*100:.2f}%",
                            level="DEBUG"
                        )
                        return False
                    else:
    ...
    def _check_enhanced_profit_levels(self, profit_pct):
        ...
        try:
            ...
            if hasattr(self.strategy, 'rsi') and self.strategy.rsi[0] > 75:
                overbought = True
                self.strategy.log(f"检测到RSI超买状态: {self.strategy.rsi[0]:.1f}", level="CRITICAL")

            # 价格偏离均线判断
            if (hasattr(self.strategy, 'ma_fast') and
                self.strategy.data.close[0] > self.strategy.ma_fast[0] * 1.15):
                overbought = True
                self.strategy.log(f"检测到价格远离均线: 当前价格为MA_fast的{self.strategy.data.close[0]/self.strategy.ma_fast[0]:.2f}倍",
                               level="CRITICAL")
        except:
            pass
    ...
    # 在 _calculate_position_size 中实现凯利公式
    ...
    def _calculate_position_size(self):
        ...
        """
        try:
            market_regime = self.market_state.get_market_regime()  # 添加这行
            trend_strength = self.market_state.get_trend_strength()  # 添加这行
            ...
            if not self.strategy.position:
                if market_regime in ["uptrend", "strong_uptrend"]:
                    base_position_pct = min(base_position_pct * 2.0, 0.6)  # 从1.5改为2.0，从0.5改为0.6
            ...
            if market_regime in ["strong_uptrend", "uptrend"] and trend_strength > 8.0:
                ...
                cash_available = self.strategy.broker.get_cash() * cash_utilization
    ...
    def _check_position_dip(self, current_price, required_dip_pct):
        ...
        self.strategy.log(
            f"加仓回调检查: 近{lookback_days}天高点={recent_high:.2f}, "
            f"当前价={current_price:.2f}, 回调幅度={dip_from_high*100:.2f}%",
            level="DEBUG"
        )
        ...
        # === 额外条件：防止追高 ===
        # 只检查最近5天内是否有急涨
        if hasattr(self, '_last_add_bar') and self._last_add_bar is not None:
            bars_since_last_add = len(self.strategy) - self._last_add_bar
            if bars_since_last_add <= 5:
                # 检查5天涨幅
                five_days_ago = self.strategy.data.close[-5] if len(self.strategy.data.close) > 5 else current_price
                recent_gain = (current_price - five_days_ago) / five_days_ago if five_days_ago > 0 else 0

                if recent_gain > 0.05:  # 5天涨幅超过5%
                    self.strategy.log(
                        f"短期涨幅过大({recent_gain*100:.1f}%)，暂缓加仓",
                        level="INFO"
                    )
                    return False
    ...
    # def _check_add_price_condition(self, current_price):
    #     """检查加仓价格条件 - 极简版本，只防止短期追高"""
    #     # 完全移除历史价格比较！
    #     # 只检查非常短期的追高情况
    #
    #     if hasattr(self, '_last_add_bar') and self._last_add_bar is not None:
    ...
    #
    #         # 只在3天内防止连续加仓
    #         if bars_since_last_add <= 3:
    #             self.strategy.log(
    #                 f"距上次加仓仅{bars_since_last_add}天，暂缓",
    #                 level="INFO"
    #             )
    #             return False
    #
    ...
    def _check_volume_and_trend_confirmation(self, trend_strength):
        """检查成交量和趋势确认"""
        try:
            # 成交量确认
            base_volume_mult = ParamAccessor.get_param(self, 'base_volume_mult')
            volume_confirmation = self.strategy.data.volume[0] > self.strategy.volume_ma[0] * (1.0 + base_volume_mult * 0.5)

            # 趋势确认
            trend_confirmation = trend_strength > 3.0 or self.strategy.data.close[0] > self.strategy.ma_slow[0]

            # 计算满足的确认条件数量
            confirmations = sum([volume_confirmation, trend_confirmation])
            required_confirmations = 1  # 降低要求

            self.strategy.log(
                f"回调确认条件详情: 成交量确认? {volume_confirmation}, "
                f"趋势良好? {trend_confirmation}, 总计: {confirmations}/{required_confirmations}",
                level="INFO"
            )
            ...
            self.strategy.log(f"回调确认检查错误: {str(e)}", level="DEBUG")
            return False
    ...
    def _check_new_position_confirmation(self, market_regime, trend_strength):
        ...
        if market_regime in ["strong_uptrend", "uptrend", "sideways"] and trend_strength > max(2.0, sideways_threshold * 0.5):
            try:
                ...
                self.strategy.log(
                    f"非ETF新建仓位条件检查: ma上升? {rising_ma}, 价格>均线? {price_above_ma}, "
                    f"MACD信号? {macd_signal}, 价格上涨? {price_up}, "
                    f"市场状态: {market_regime}, 趋势强度: {trend_strength:.1f}",
                    level="INFO"
                )
...
class EnhancedStrategy(bt.Strategy):
    ...
    def log(self, txt, dt=None, level='INFO'):
        ...
        if level == 'CRITICAL':
            ...
            noise_keywords = [
                ...
                '新仓位保护期','新建仓位满足回调条件',  # 新增：过滤重复的回调检查  # 过滤保护期的每日重复提示
                '卖出冷却期'  # 过滤冷却期的重复提示
            ]
    ...
    def next(self):
        """Enhanced main strategy logic with more trading opportunities"""
        ...
        if not self.position:
            # 确保position_manager的状态被重置
            if hasattr(self, 'position_manager'):
                # 手动调用重置方法
                self.position_manager._check_position_reset()

        if len(self.data) % 100 == 0:  # 每100个bar
        ...
        if len(self.data) % 365 == 0:
            ...
            if hasattr(self, '_last_crash_bar'):
                ...
                if bars_since_crash > 90:  # 超过90天
                    delattr(self, '_last_crash_bar')
                    self.log(f"强制清除过期崩盘标记 - 已{bars_since_crash}天", level="CRITICAL")
        ...
        if len(self.data) % 50 == 0:
            ...
            risk_status = "正常"
            if hasattr(self.risk_manager, '_drawdown_triggered') and self.risk_manager._drawdown_triggered:
                days_in_limit = len(self.data) - getattr(self.risk_manager, '_drawdown_trigger_day', len(self.data))
                risk_status = f"风险限制中({days_in_limit}天)"

            self.log(f"状态检查 - 日期:{current_date}, 持仓:{position_size}, "
                    f"账户价值:${portfolio_value:.2f}, 风险状态:{risk_status}", level="INFO")

        # 参数验证（只在第一个K线执行）
        if len(self.data) == 1:
            critical_params = ['stop_atr', 'trail_stop_mult', 'td_count', 'profit_trigger_1']
            for param in critical_params:
                if hasattr(self.trading_params, param):
                    value = getattr(self.trading_params, param)
                    self.log(f"参数验证 - {param}={value}")
        ...
        if len(self.data) > 5:
            ...
            # 检测异常状态
            if abs(current_position_size) > 20000:  # 持仓异常大
                self.log(f"检测到异常持仓: {current_position_size}股，强制平仓", level='CRITICAL')
                if self.position:
                    self.order = self.close()
                    return

            if portfolio_value < 0:  # 资金为负
        ...
        try:
            ...
            # 添加调试日志
            if risk_limited and not self.position:
                # 无仓位时也输出日志，帮助调试
                self.log(f"风险限制阻止开仓 - 当前bar: {len(self)}", level='CRITICAL')
            ...
            # 在任何买入决策之前检查市场崩盘
            if self.market_state.is_market_crash():
            ...
            # 检查是否从崩盘中恢复
            if hasattr(self, '_last_crash_bar') and self.market_state.is_market_recovering():
                self.log("市场显示恢复迹象", level="INFO")
                # 市场恢复后可以继续正常交易逻辑

            if self.position:
                # 确保high_since_entry正确初始化和更新
                if not hasattr(self, 'high_since_entry') or self.high_since_entry == 0:
                    self.high_since_entry = self.data.high[0]  # 使用最高价初始化
                else:
                    self.high_since_entry = max(self.high_since_entry, self.data.high[0])

            # Track daily information
            current_date = self.data.datetime.date(0)
            ...
            # Position management - different logic path for with/without position
            if self.position:
                self._was_in_position = True  # 添加这行
                # Check for stop loss exit
                if self.stop_manager.check_all_stops():
                    self.log(f"Stop loss triggered - price: {current_price:.2f}", level='INFO')
                    self.order = self.close()
                    return
                ...
                if position_action['action'] == 'buy':
                    ...
                    self.log(f"【准备卖出】 - 原因: {position_action['reason']}, 数量: {position_action['size']}股", level="INFO")
                    # 添加条件：小额获利了结时跳过
                ...
                if self.market_state.is_market_crash():
                    ...
                    if not hasattr(self, '_last_crash_bar'):
                        self._last_crash_bar = len(self)
                    return
                elif self.market_state.is_market_recovering():
                    # 市场正在恢复，可以考虑买入
                    self.log("市场显示恢复迹象", level="INFO")
                ...
                # 检查回调条件
                dip_check = self.position_manager._should_buy_the_dip(current_price)
                # ===== 核心修改：如果td_count > 0，则必须有TD信号支持 =====
                if td_count > 0:
                    # TD序列是必要条件
                    td_satisfied = hasattr(self.market_state, 'td_up') and self.market_state.td_up >= td_count
                    if not td_satisfied:
                        self.log(
                            f"TD序列未满足({getattr(self.market_state, 'td_up', 0)} < {td_count})，跳过所有买入信号",
                            level="INFO"
                        )
                        return
                # 记录信号来源
                ...
                should_buy = False
                if td_count > 0:
                    # TD模式：必须有base_buy（包含TD检查）
                    should_buy = base_buy
                    if should_buy and dip_check:
                        signal_source.append("(回调加强)")
                else:
                    # 非TD模式：base_buy或dip_check都可以
                    should_buy = base_buy or dip_check
                ...
                if should_buy:
                    ...
                    if hasattr(self, 'last_trade_bar'):
                        ...
                        if bars_since_last < min_bars:
                            self.log(f"交易间隔不足: {bars_since_last} < {min_bars}，跳过", level="INFO")
                            return
            ...
            self.log(f"Strategy next() error: {str(e)}", level='INFO')
            raise  # 让错误暴露出来，而不是静默返回0
        ...
        if trade.isclosed:
            try:
                ...
                if trade.pnl < 0:
                    ...
                    self.consecutive_losses = 0
                    self.last_trade_was_loss = False
                    self.log(f"盈利交易 - 重置连续亏损计数", level="INFO")
    ...
    def _convert_date(self, date_val):
        ...
        try:
            ...
            return str(date_val)  # 如果转换失败，至少保留原始值的字符串表示
    ...
    try:
        ...
        for col in required_cols:
            if col not in df.columns:
                raise Exception(f"YFinance数据缺少必需列: {col}")
    ...
    try:
        ...
        api_sources = [
            ("Alpha Vantage", lambda: alphavantage_download(
                ...
                api_key=ALPHA_VANTAGE_API_KEY  # 使用预定义的密钥
            )),
            ("Twelve Data", lambda: twelvedata_download(
                ...
                interval=interval_input,
                api_key=TWELVE_DATA_API_KEY  # 使用预定义的密钥
            )),
